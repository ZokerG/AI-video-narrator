...
def create_reel_video(
    ...
    Applies zoom effect (Ken Burns) to static images.
    """
    from moviepy import ImageClip, CompositeVideoClip, ColorClip
    # In MoviePy v2, effects are usually in moviepy.video.fx or vfx
    # We will try to import vfx safely
    try:
    ...
    for i, scene in enumerate(scenes):
        ...
        # We need to find the audio file corresponding to this scene index or map
        # Assuming audio_map and scenes are aligned by index
        if i < len(audio_map):
            audio_info = audio_map[i]
            # Use actual audio duration if available, else estimated
        ...
        if not img_path or not os.path.exists(img_path):
            ...
            # Create Image Clip
            # 1. Load and Resize
            # We want to Fill height, then crop center
            clip = ImageClip(img_path).with_duration(duration)
            
            # Init resize to cover screen height
            # If aspect ratio is wider than 9:16, resize by height
            img_w, img_h = clip.size
            ratio = img_w / img_h
            target_ratio = W / H
            
            if ratio > target_ratio:
                # Wider image: resize based on height
                new_h = H
                new_w = int(img_w * (H / img_h))
                clip = clip.resized(height=H)
            else:
                # Taller/Narrower image: resize based on width (rare for landscape sources)
                new_w = W
                new_h = int(img_h * (W / new_w))
                clip = clip.resized(width=W)
                
            # Center Crop
            clip = clip.cropped(width=W, height=H, x_center=new_w/2, y_center=new_h/2)
            
            # Apply Zoom Effect (Simple Scale Factor)
            # v2.0 uses with_effects([vfx.dataset(...)]) or transforms
            # Simple linear zoom: t=0 -> 1.0, t=end -> 1.1
    ...
    if background_track and os.path.exists(background_track):
        ...
        # Volume (Manual approximate since e.g. .volumex is deprecated in some v2 builds)
        # Let's trust moviepy v2 API for audio levels if available, or just mix it
        # For now, simplistic approach: assuming ffmpeg will handle mixing safely
        # Note: MoviePy v2 audio mixing can be complex.
        # We'll use the temp wav trick if needed, or composite.
        # We'll use the temp wav trick if needed, or composite.
        # from moviepy import CompositeAudioClip (Removed to avoid UnboundLocalError)
        
        # Lower volume by creating a CompositeAudioClip with lower weight? No.
        # Use previous mix_audio logic approach if precise control needed.
        # For this function, let's keep it simple: just add it. User can mix later?
        # Ideally we apply volume reduction.
        # new_audio = bg.with_effects([afx.MultiplyVolume(0.1)]) # If afx exists
